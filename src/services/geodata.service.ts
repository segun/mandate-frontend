...
export const geodataService = {
    ...
    ): Promise<PaginatedResponse<GeoLga>> {
        const response = await api.get(`/geodata/states/${stateId}/lgas`, {
            params: { page, limit },
        });
        return response.data;
    },
    ...
    async getWardsByLga(
        ...
        page = 1,
        limit = DEFAULT_PAGE_LIMIT,
    ): Promise<PaginatedResponse<GeoWard>> {
        const response = await api.get(`/geodata/lgas/${lgaId}/wards`, { params: { page, limit } });
        return response.data;
    },

    // Get reference wards by state
    async getWardsByState(
        stateId: string,
        page = 1,
        limit = DEFAULT_PAGE_LIMIT,
    ): Promise<PaginatedResponse<GeoWard>> {
        const response = await api.get(`/geodata/states/${stateId}/wards`, {
            params: { page, limit },
        });
        return response.data;
    },
    ...
    async getAllPollingUnits(
        page = 1,
        limit = DEFAULT_PAGE_LIMIT,
    ): Promise<PaginatedResponse<GeoPollingUnit>> {
        const response = await api.get("/geodata/polling-units", { params: { page, limit } });
        return response.data;
    },

    // Get reference polling units by ward
    async getPollingUnitsByWard(
        wardId: string,
        page = 1,
        limit = DEFAULT_PAGE_LIMIT,
    ): Promise<PaginatedResponse<GeoPollingUnit>> {
        const response = await api.get(`/geodata/wards/${wardId}/polling-units`, {
            params: { page, limit },
        });
        return response.data;
    },
    ...
    async getPollingUnitsByLga(
        lgaId: string,
        page = 1,
        limit = DEFAULT_PAGE_LIMIT,
    ): Promise<PaginatedResponse<GeoPollingUnit>> {
        const response = await api.get(`/geodata/lgas/${lgaId}/polling-units`, {
            params: { page, limit },
        });
        return response.data;
    },
    ...
    async getPollingUnitsByState(
        stateId: string,
        page = 1,
        limit = DEFAULT_PAGE_LIMIT,
    ): Promise<PaginatedResponse<GeoPollingUnit>> {
        const response = await api.get(`/geodata/states/${stateId}/polling-units`, {
            params: { page, limit },
        });
        return response.data;
    },
